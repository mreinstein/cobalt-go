...
fn corners(i: u32) -> vec2<f32> {
  var p: vec2<f32>;
    
  // 3-vertex fullscreen triangle
  switch i {
    case 0u: { p = vec2<f32>(-0.5, -0.5); }
    case 1u: { p = vec2<f32>(0.5, -0.5); }
    case 2u: { p = vec2<f32>(-0.5,  0.5); }
    default: { p = vec2<f32>(0.5,  0.5); }
  }

  return p;
}

fn uvBase(i: u32) -> vec2<f32> {
  var p: vec2<f32>;
...
fn fs_main(in : VSOut) -> @location(0) vec4<f32>  {
  let texel = textureSampleLevel(uTex, uSampler, in.uv, 0.0);
