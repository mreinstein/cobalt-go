...
fn vs_main (@builtin(instance_index) i_id : u32,
    ...
    var vertexPosition = fullscreen_pos(VertexIndex);
    var vertexTexCoord = fullscreen_uv(VertexIndex);
...
fn fs_main (@location(0) TexCoord: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    var tile = floor(textureSampleLevel(tileTexture, tileSampler, tileFoo, 0.0) * 255.0);
